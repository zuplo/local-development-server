// Originally from https://github.com/kenchris/urlpattern-polyfill
// Updated to mimic the behavior on Cloudflare
...
function tokensToRegexp(tokens, keys, options = {}) {
  ...
  const endsWith = `[${escapeString(
    options.endsWith === void 0 ? "" : options.endsWith
  )}]|$`;
...
function tokensToPattern(tokens, options) {
  ...
  for (let i = 0; i < tokens.length; ++i) {
    ...
    const optionsPrefixes =
      options.prefixes !== void 0 ? options.prefixes : "./";
    let needsGrouping =
      token.suffix !== "" ||
      (token.prefix !== "" &&
...
// NCHEN: Cloudflare does not take an options, but it defaults to {ignoreCase: true}
// So we mimic this here
...
// NCHEN: Replace the global one with this
